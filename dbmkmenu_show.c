...
readstdin(void)
{
	...
	while ((len = getline(&line, &linesiz, stdin)) != -1) {
		...
		if (!(items[i].text = strdup(line)))
			die("strdup:");
		if (showall) {
			items[i].stext = items[i].text;
		} else {
			for (ssize_t i = 0; i < len; i++) {
				if (line[i] == bookmarkseparator) {
					len = i + 1;
					line[len - 1] = '\0';
					break;
				}
...
setup(void)
{
	...
	XClassHint ch = {"dbmkmenu", "dbmkmenu"};
#ifdef XINERAMA
...
usage(void)
{
	die("usage: dbmkmenu_show [-Abfiv] [-l lines] [-p prompt] [-fn font] [-m monitor]\n"
...
main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++)
		...
		if (!strcmp(argv[i], "-v")) {      /* prints version information */
			puts("dbmkmenu-"VERSION);
			exit(0);
		} else if (!strcmp(argv[i], "-b")) /* appears at the bottom of the screen */
			topbar = 0;
		else if (!strcmp(argv[i], "-f"))   /* grabs keyboard before reading stdin */
			fast = 1;
		else if (!strcmp(argv[i], "-A"))   /* print instead of everything before '>' and no comments */
			showall = 1;
